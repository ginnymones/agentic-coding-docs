...
7. **Infrastructure Decisions** — Before writing code, lock in: auth method, database, and hosting platform. Confirm connection strings work and env vars are documented. Don't build UI on unverified infrastructure.
...
- **Verify at checkpoints**: After setting up infrastructure, confirm it works (can I connect to the DB? can I authenticate?) before building features on top. After each major feature, deploy and test on a real device.
...
- Environment variables documented in `.env.local.example` from day one
- `.gitignore` verified to cover all generated folders (node_modules, build output) in all subdirectories
- Connection strings and external service access tested before first deploy
